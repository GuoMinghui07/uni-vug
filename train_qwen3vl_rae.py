...
def main():
    ...
    gan_start_epoch = float(loss_cfg.get("disc_start", 0))
    disc_update_epoch = float(loss_cfg.get("disc_upd_start", gan_start_epoch))
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for step, (images, _) in enumerate(loader):
            ...
            with autocast(**autocast_kwargs):
                ...
                if images.dim() == 4 and recon.dim() == 5:
                    recon_first = recon[:, 0]
                    recon_second = recon[:, 1] if recon.shape[1] > 1 else recon_first
                    rec_loss = 0.5 * (
                        (recon_first - images).abs().mean()
                        + (recon_second - images).abs().mean()
                    )
                    recon_for_lpips = recon_first
                else:
                    rec_loss = (recon - images).abs().mean() # L1
                    recon_for_lpips = recon
                recon_normed = recon_for_lpips * 2.0 - 1.0
                if use_lpips:
                ...
                for _ in range(disc_updates):
                    ...
                    with autocast(**autocast_kwargs):
                        ...
                        with torch.no_grad():
                            ...
                            if images.dim() == 4 and recon_disc.dim() == 5:
                                recon_disc = recon_disc[:, 0]
