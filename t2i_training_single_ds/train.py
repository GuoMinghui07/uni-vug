...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--config", type=str, default="training/config/dit_training.yaml", help="YAML config path.")
            ...
            for micro_idx in range(accumulation_steps):
                ...
                if context_dropout_prob > 0 and ddp_model.training:
                    ...
                    if drop_mask.any():
                        ...
                        tokenized = ddp_model.module.mllm_encoder.tokenize(cfg_texts)
                        tokenized = {
                            k: (v.to(device=device, non_blocking=True) if torch.is_tensor(v) else v)
                            for k, v in tokenized.items()
                        }
